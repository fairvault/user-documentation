...
# Red (Metadata only) sensitivity level: highly sensitive or highly confidential data with strict curatorial control

A dataset must be classified with a red (Metadata only) sensitivity level when it contains data that poses substantial legal, or public risk if released without unconditional safeguards.  
...
> Access to datasets classified under the red sensitivity level is always evaluated case-by-case by data curators.  
...
If there are high sensitivity or confidentiality risks, the dataset may get a **Red (Metadata only)** sensitivity level.
...
## Checklist when depositing
 ...
 - [ ] Upload the dataset files AND **implement restriction on the dataset files**. Do not allow access requests. See [instructions below](#implementing-access-restriction-to-data-files-for-datasets-with-a-red-sensitivity-level).
...
## Implementing access restriction to data files for datasets with a Red sensitivity level
...
- Uncheck the box that says "Enable access request"

  <img src="https://fairvault.github.io/user-documentation/docs/guides/images/FV_enable_access_request_red.png" alt="enable_access_request" width="700"/>
